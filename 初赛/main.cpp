...
vector<int>edge_line;
bool edge_line_mark[maxd], edge_line_mark0[maxd];
...
void copy_loop_times(int app_a, int app_b,int mark) {//a为b的前驱
	...
	if (nowwindow < window_looptype1 && prewindow < window_looptype1) {
		if (apparatus_status[app_a].loop_times[window_looptype1] == apparatus_status[app_b].loop_times[window_looptype1]) {
		...
		else if (apparatus_status[app_a].loop_times[window_looptype1] > apparatus_status[app_b].loop_times[window_looptype1]) {
			apparatus_status[app_b].pre_window = nowwindow;
			apparatus_status[app_b].mark = mark;
		}
	}
	else if (nowwindow == prewindow) {
		if (apparatus_status[app_a].loop_times[app_a] == apparatus_status[app_b].loop_times[app_b]) {
			apparatus_status[app_b].mark = min(apparatus_status[app_b].mark, mark);
		}
		else if (apparatus_status[app_a].loop_times[app_a] > apparatus_status[app_b].loop_times[app_b]) {
			apparatus_status[app_b].pre_window = nowwindow;
			apparatus_status[app_b].mark = mark;
		}
	}
...
void install(int app_id, int pre_window) {
	...
	if (pre_window == -1)install_dfs(app_id, pre_window + 1);
	else if (apparatus_status[app_id].mark) {
		install_dfs(app_id, pre_window);
	}
	else if (self_loop[pre_window]&&loop_times[pre_window] < L &&now_flag) {//判断能否自回环安装
		LL mincost = -1;
		int install_area = -1;
...
void output() {
	...
	int cnt = 0;
	int cnt2 = 0;
	for (int i = 0; i < D; i++)if (edge_line_mark0[i])cnt++;
	cout << cnt << "\n";
	for (auto ed : edge_line) {
